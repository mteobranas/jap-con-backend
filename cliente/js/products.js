...
function sortCategories(criteria, array) {
  ...
  if (criteria === ORDER_ASC_BY_COST) {
    ...
    result = array.sort(function (a, b) {
      let aCount = parseInt(a.sold_count)
      let bCount = parseInt(b.sold_count)
...
function showCategoriesList() {
  ...
  for (let i = 0; i < currentCategoriesArray.length; i++) {
    ...
    let image = JSON.parse(product.images)
    ...
    if (
      ...
      htmlContentToAppend += `
            <div onclick="setProdID(${product.id})" class="list-group-item list-group-item-action cursor-active" id="${product.id}">
                <div class="row">
                    <div class="col-3">
                        <img src="${image[0]}" alt="${product.name}" class="img-thumbnail">
                    </div>
                    <div class="col">
                        <div class="d-flex w-100 justify-content-between">
                            <h4 class="mb-1">${product.name} | ${product.cost}</h4>
                            <small class="text-muted">${product.sold_count} artículos</small>
                        </div>
